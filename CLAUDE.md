...
| 004 | analysis | IN_PROGRESS | 단일 종목 분석 — premium·matrix·orderbook·slippage·arbitrage (BE 전용) |
| 005 | history | IN_PROGRESS | Influx `premium` 점 규칙·`/history/*`·백필 + history 탭 (쓰기는 009) |
| 006 | wallet-status | IN_PROGRESS | 거래소 입출금 상태·망 기준 판정 → 스프레드 표에 반영 |
| 007 | deploy | IN_PROGRESS | Docker·compose(server·web·influxdb·redis)·CI·EC2 배포 |
...
| 010 | raw-archive | DONE | 거래소 원문(WS 프레임·REST 응답) 전량을 거래소별 60초 객체로 S3 `raw/` 에 저장 (BE 전용) |
...
실행 순서 = 번호 순. 단 재구축 중인 스펙(IN_PROGRESS·TODO)은 **004 → 005 → 006 → 007** 순으로 돈다 — 001·003·009·010·011·012(DONE)가 토대(행·틱·계약·3거래소 스트림·김프 표·틱 저장 3계층·원문 아카이브·실패 이력)이고 나머지가 그 위에 얹힌다.
