...
marketlens/
  ...
  docs/
    ...
    specs/                    기능 계약 문서 (갱신형 — 그 기능의 현재 계약. 역사는 git 에)
      TEMPLATE.md
      NNN-<name>.md
...
5. 죽은 문서·문구는 지운다. 역사는 git 커밋에 있다.
...
실행 순서 = 번호 순.
상태: TODO(합의된 계약, 미구현) → IN_PROGRESS → DONE(구현·검증됨).
**스펙은 살아있는 계약이다 — DONE 이후에도 동작을 바꿀 땐 그 기능의 스펙을 직접 고친다.** 스펙을 고치는 순간 코드·테스트가 낡은 것이므로, 같은 PR 에서 코드·테스트를 스펙에 맞추고 그 기능의 §4 검증을 다시 통과시킨다(§6 루프). 여러 기능에 걸친 변경은 걸치는 스펙을 각각 고친다. 낡은 문구를 "예전엔 ~였다" 로 남기지 않는다 — 역사는 git 이 든다.
...
허용:
- 수정 가능: `server/` `web/` `docs/context/*` `docs/runbooks/*` 담당 스펙(상태 무관 — §4 살아있는 계약 규칙대로) `CLAUDE.md` 스펙 인덱스 상태. `docs/design/*` 원본 변경은 사람 합의 후.
...
2. **구현된 기능의 스펙·context 문서**: 스펙 수정 = 동작 변경 요청이다. 같은 PR 에서 코드·테스트를 스펙에 맞추고, 그 기능의 §4 검증을 **다시 실행**해 통과를 확인한 뒤에만 커밋한다 — server `ruff check . && pytest -q`, web `npm run lint && npm run build`, 해당되면 curl 스모크. 실패 = 문서↔코드 어긋남이므로 옳은 쪽을 사람이 정하고(§3-3) 맞춘다.
