...
// 화면의 봉 8종은 계층 하나를 골라 그 안에서 접는다(rollup.ts). 청크(360창) 단위로
// (쌍, 방향, 계층, 청크 시작) 키에 받아 두고, 없는 청크만 새로 부른다.
...
/** 차트에서 고를 수 있는 해외 거래소 — 서버가 수집하는 해외 거래소는 binance·bybit·bitget(019·020). `mock` 은 015 UI 시안용:
...
/** 청크 길이(창 수) — 첫 화면 봉 수(`INITIAL_BARS`)와 같게 둔다. 서버 상한(1,440창, §3.6)을 그대로 청크로 쓰면
 *  첫 로드가 필요한 양의 4배(1m 하루 청크 ≈500KB × 쌍 6개)를 받아 느렸다. 창 길이의 배수라 창이 청크에 걸치지 않는다. */
export const CHUNK_WINDOWS = 360
export const chunkSec = (res: Res) => CHUNK_WINDOWS * RES_SEC[res]
...
/** ts 가 속한 청크의 시작 — KST 기준 청크 길이의 배수로 정렬(1m 은 6시간 단위, 1d 는 360일 단위). */
export function chunkStart(ts: number, res: Res): number {
  const L = chunkSec(res)
