...
def test_three_clients_receive_the_same_frames() -> None:
    ...
    with TestClient(app) as client:
        ...
        try:
            assert all(ws.receive_json()["type"] == "waiting" for ws in sockets)
            client.portal.call(bus.publish_table, json.dumps(table([row("BTC")])))
            client.portal.call(
                bus.publish_table, json.dumps(table([row("BTC", fwd=3.0)]))
            )
            frames = [[ws.receive_text(), ws.receive_text()] for ws in sockets]
            assert frames[0] == frames[1] == frames[2]
            assert json.loads(frames[0][1])["rows"][0]["fwd"] == 3.0
        finally:
            for ws in sockets:
