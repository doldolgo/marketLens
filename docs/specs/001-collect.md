...
- 바이낸스 심볼 집합은 core 계약 하나로 받는다 — `refresh(client) -> int`(REST 로 목록을 갱신하고 나간 호출 수, 실패는 거래소 예외), `bases() -> set[str]`(현재 아는 USDT 현물 base), `set_universe(bases)`(확정된 우주를 넘겨 구독 대상을 맞춘다 — 매초 부르며 바뀐 게 없으면 무동작). 012 가 구현하고 `main.py` 가 꽂는다. 빈 집합을 주는 기본 구현(테스트용)이 꽂히면 우주가 비고 **행이 하나도 저장되지 않는다**(USDT 시세는 구독하므로 갱신된다).
