...
### 3.5 web — 기록 탭
데이터는 `GET /history/events` 하나. 기간·거래소 필터가 바뀔 때 1회 조회(디바운스 400ms·진행 중 요청 취소, 005 §3.6 방식), 그리고 **60초마다** 같은 조건으로 다시 불러 진행 중 사건의 지속·최대값을 갱신한다. 유형·심볼 선택은 클라이언트에서 거른다. 항상 `start`·`end` 를 붙인다.
- 필터바: 기간 `1주/1달/3달`(기본 1주), 유형 `전체/김프만/역프만`, 거래소 `전체/업비트/빗썸`(전체 = `dom` 생략). 기준 입력은 없다 — 우측 설명 `사건 = 원값 스프레드 1.0% 진입 → 0.5% 이탈, 1분 이하 제외 · 기간 내 N건`.
- 좌 카드 "티커별 사건 통계 · {기간} — 열 클릭으로 정렬": 열 `티커|횟수|최대 지속|평균 지속|최대 김프|평균 김프|최대 역프|평균 역프|최신`, 응답을 심볼별로 집계(거래소 전체면 두 거래소 사건을 합쳐 센다), 상위 30행, 헤더 클릭 정렬(재클릭 반전, 기본 횟수 내림차순, 없는 방향은 `–`·뒤로). 행 클릭 → 선택 심볼. 선택 행 accent 배경. 비면 `기간 내 사건 없음`.
- 우 column: 요약 카드(선택 심볼, 총 사건·김프/역프 수, 평균·최장 지속, 기간 점유율 = Σ지속/기간). 타임라인 2줄(김프 accent/역프 neutral, 위치·폭 = 기간 대비, 최소폭 보장, 진행 중은 지금까지, 축 라벨 5등분 `M/D`). "사건 로그 · {심볼} 최근 20건"(유형|거래소|시작|종료(진행 중이면 `진행 중`)|지속|최대 스프레드 — 김프 POS 색, 역프 NEG 색).
- 선택 심볼: 초기 `'BTC'`, 스프레드 행 클릭 피벗 그대로. 선택 심볼에 사건이 없으면 요약은 `–`, 로그는 `기간 내 사건 없음`.
- 상태: 조회 중 `조회 중…`(직전 유지), 오류 `기록을 불러오지 못했습니다 (HTTP n)`. 표 구조·색·간격은 `docs/design/reference/tabs/HistoryTab.tsx`·`theme.css`.
...
- 수동(EC2): 배포 후 알트 하나가 1% 를 넘는 사건이 열리고 60초 뒤 Influx UI 에 `premium_event` 1점, 닫힌 뒤 `end_ts` 채워짐. 기록 탭 1주 표에 그 코인이 뜨고 행 클릭 → 우측 로그에 시작·종료. 서버 재기동 직후 진행 중이던 사건이 표에 그대로 남는다. 요청 URL 에 `start`·`end` 가 있다. 서버 테스트·lint, web build·lint 통과.
