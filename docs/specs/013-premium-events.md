...
- 필터바 아래 차트 카드(심볼 검색 → Enter, 봉 종류 1m~1d, 국내·해외 거래소 체크박스 다중 선택, 김프 캔들/쌍별 선·USDT 기준 가격·입출금 띠)는 **014 시안으로 mock 데이터**를 그린다(`MOCK` 배지). 이 스펙의 API 와 무관하고, 서버 계약·실데이터 연결은 014 가 정한다.
