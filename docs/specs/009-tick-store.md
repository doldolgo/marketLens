...
- 007: 배포 compose 는 server·web·influxdb·redis 4컨테이너, 호스트 노출은 web 하나(Influx·Redis 는 비공개). server 의 컨테이너 내부 주소는 `environment` 로 덮어쓴다(`INFLUX_URL`·`REDIS_URL`).
...
- 앱 종료 시: 틱 루프가 마지막 틱을 인계한 뒤, 큐에 남은 틱을 Redis 로 보내려 **한 번씩** 시도하고(불달이면 §3.3 규칙대로 버림) 보내기 태스크를 닫는다. 이 비우기는 **총 5초**(명령당 소켓 타임아웃 1회분, 코드 상수) 안에서만 시도한다 — 넘으면 남은 틱은 전부 버리고 버린 개수를 경고 1줄로 남긴다. Redis 가 무응답이면 틱마다 타임아웃을 기다리게 되는데, 종료를 큐 길이만큼 붙들지 않기 위해서다(`docker stop` 은 10초 뒤 강제 종료한다).
...
### 3.5 계층 ③ — flusher (60초)
주기 60초는 코드 상수. 기동 후 먼저 60초 잔 뒤 첫 회차. **회차 = 페이지 반복** — 스트림 전량을 1,000건 페이지(코드 상수)로 잘라 한 페이지씩 끝낸다. 메모리에 드는 것은 한 번에 한 페이지뿐이라 밀린 구간이 몇 시간이어도 메모리는 페이지 크기에 비례한다(24시간 4,200만 점을 한 번에 펼치면 수십 GB 가 되어 EC2 에서 죽는다). 페이지마다:
1. `XRANGE ticks <직전 페이지 마지막 ID 다음> +` 로 1,000건을 읽는다. 첫 페이지가 비면 이번 회차 생략(쓰기 0회, XDEL 0회).
2. 그 페이지 틱의 `rows` 를 `premium` 점으로, `dwFailed` 를 `dw_fail` 점으로 바꾼다. 각 점의 `time` 은 그 틱의 `ts`. 5,000점 배치(코드 상수)로 나눠 쓰고, **모든 배치가 성공해야 페이지 성공**. 디코드·점 생성·쓰기는 스레드에서 돌려 이벤트 루프(틱 루프·스트림 펌프)를 막지 않는다.
3. 페이지 성공 → **그 페이지의 엔트리 ID 만** `XDEL` 한다. **Redis 를 비우는 시점은 그 엔트리의 Influx 쓰기가 끝난 뒤뿐이다.** 읽은 뒤 새로 들어온 엔트리는 지우지 않는다(ID 로만 지운다).
4. 1,000건 미만인 페이지를 끝내면 회차 성공. 아니면 다음 페이지로.
5. 어느 페이지든 실패(읽기·쓰기·XDEL) → **회차 중단**, 그 페이지와 그 뒤는 손대지 않고 `DB 저장 실패 (연속 n회)` 로그. 앞서 지운 페이지는 이미 Influx 에 있다. 다음 회차가 지우지 못한 페이지부터 다시 보낸다. Influx 가 같은 (태그, 시각) 을 덮어쓰므로 중복 적재는 무해하다 — 실패는 구멍이 아니라 지연이다.
...
- **잘림 감지**: 회차가 실패해 지우지 못한 구간은 다음 회차에 **같은 첫 ID** 부터 다시 읽혀야 한다. 기준은 실패한 회차에서 **지우지 못한 첫 엔트리의 ID**(실패한 페이지의 첫 ID). 다음 회차의 첫 ID 가 그것과 다르면(빈 스트림도 "다름") 오래된 쪽이 `MAXLEN` 에 잘린 것이므로 경고 로그 1줄(`Redis 스트림 잘림`). 성공한 회차와 빈 회차는 읽은 구간을 전부 지웠으므로 기준이 없다. **XDEL 도중 실패는 잘림 판정 대상이 아니다** — 명령이 Redis 에 닿았는지 알 수 없어 기준을 남기지 않는다. 읽기 자체가 실패해 첫 ID 를 모르면 직전 기준을 유지하되, 이번 회차에 이미 지운 페이지가 있으면 기준을 비운다.
- 빈 회차는 읽기 성공이라 연속 실패를 0 으로. 연속 실패 뒤 회차가 성공하면 그 회차가 옮긴 틱 수를 info 로그 1줄(`밀린 틱 n건`)로 남기고 연속 횟수를 0 으로.
...
- 값은 링버퍼에 넣는 순간 `/spreads` 응답 자리(소수 3자리, 003 §3.2)로 반올림한다 — 틱마다 바뀌는 값은 조합당 1개뿐인데 표는 매초 전 행 × 30개를 싣기 때문이다. 원값은 Influx `premium` 에 남는다. 복원(아래)도 같은 자리로 넣는다.
...
- **Influx 불달**: Redis 에 계속 쌓이고, 복구되면 밀린 구간이 한 회차에 페이지 단위로 들어간다(24시간 상한 안에서 무유실, 메모리는 페이지 크기에 비례).
...
- 종료 시 큐 비우기는 총 데드라인 안에 끝난다 — Redis 가 틱마다 느리게 실패해도 데드라인이 지나면 남은 틱을 버리고 개수를 경고 1줄로 남긴다(큐 0).
...
- 회차 성공 후 스트림 길이 0. 읽은 뒤에 들어온 엔트리는 남는다.
- 회차 실패 → 실패한 페이지는 스트림 그대로, 다음 회차가 같은 구간을 다시 보내고 그때 비운다. 실패 로그에 연속 횟수.
- 한 배치가 실패하면 그 페이지는 지우지 않는다(한 페이지 구간이면 아무것도 지우지 않는다). 배치 상수보다 큰 구간도 전부 적재된다.
- 페이지 여러 개 구간: 첫 페이지의 쓰기·XDEL 이 끝난 뒤에야 둘째 페이지를 읽는다(둘째 페이지를 읽는 시점의 스트림 길이 = 전체 − 페이지 크기). 둘째 페이지가 실패하면 첫 페이지는 지워져 있고 Influx 에 있으며, 다음 회차는 둘째 페이지부터 다시 보내고 잘림 경고가 없다.
- 같은 구간을 두 번 적재해도 점 수·값이 불변(멱등).
- 잘림 감지: 회차 실패 뒤 다음 회차의 첫 ID 가 지우지 못한 첫 ID 와 다르면 경고 로그(`Redis 스트림 잘림`). 같으면 경고 없음. 실패 뒤 스트림이 비어 있어도 경고 1줄이고 그 빈 회차는 연속 실패를 0 으로 되돌린다. XDEL 이 예외를 내면(Redis 가 지운 뒤 응답만 못 받은 경우 포함) 다음 회차에 경고가 없다.
...
# 370 passed, 1 warning in 2.94s   (009 추가분 31개: test_tick_store 23 · test_spark 6 · test_tick_store_history 2, spreads +1)
...
- 추측한 지점 (묻지 않고 정한 사소한 것) / 실행 중 함께 고친 스펙 절:
  - §3.3 인계 함수는 인코딩·spark·큐 삽입 예외도 삼킨다(로그 1줄). 종료 시 큐에 남은 틱을 한 번씩 보내 보되 총 5초(명령 타임아웃 1회분) 안에서만 — 큐 길이 × 타임아웃만큼 종료를 붙들지 않는다. 넘으면 남은 틱은 버리고 개수를 경고 1줄로.
  - §3.4 Redis 클라이언트 타임아웃(연결 2초·명령 5초)·자동 재시도 없음·기동 `PING` 1회. 엔트리 JSON 키는 `ts`·`rows`·`dwFailed` 세 개.
  - §3.5 회차를 1,000건 페이지 반복으로 — 페이지마다 읽기 → 스레드에서 디코드·5,000점 배치 쓰기 → 그 페이지 ID 만 XDEL. 전량을 한 번에 펼치면 4GB EC2 에서 2~3시간 밀린 구간만으로 OOM 이라(490행 틱 1시간 ≈ 177만 점 ≈ 1.2GB 실측) 메모리를 페이지 크기에 묶었다. 페이지 실패 = 회차 중단, 앞 페이지는 이미 Influx 에 있다.
  - §3.5 잘림 감지 규칙을 구체화 — 실패 회차에서 지우지 못한 첫 ID(실패한 페이지의 첫 ID)를 기억해 다음 회차의 첫 ID 와 비교(빈 스트림도 "다름"). XDEL 단계 실패는 지워졌는지 알 수 없어 기준을 남기지 않는다(허위 잘림 경고 방지). 빈 회차는 읽기 성공이라 연속 실패를 0 으로. Redis 읽기·XDEL 실패도 회차 실패로 같은 로그. 연속 실패 뒤 성공은 info 1줄(그 회차가 옮긴 틱 수). `INFLUX_TOKEN` 없으면 flusher 미기동.
  ...
  - 구조: 모듈 3개(`redis_stream`·`spark`·`tick_store`) 로 나눔. `RedisTickStream.read_page` 가 flusher 의 페이지 단위이고 `read_all`·`length()` 는 테스트·스모크용 보조. `FakeInflux` 는 `tests/conftest.py`(공용 fake 자리). 기동 테스트(009·012 둘 다)는 `REDIS_URL` 을 거부 포트 `redis://127.0.0.1:1/0` 로 고정한다 — dev compose 의 Redis 가 떠 있어도 테스트가 실 소켓을 열지 않는다(§3.4).
- 남은 빚:
