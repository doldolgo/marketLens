...
### 3.8 web — 기록 탭
- **차트 읽기 줄**(014 §3.7 의 거래소별 입금·출금 칸 옆): 망을 한 칸 더 보여준다. 김프 `{netFx} → {netDom}`, 역프 `{netDom} → {netFx}`(경로 방향대로). 한쪽만 있으면 있는 쪽만(`– → Ethereum`), 둘 다 없으면 `네트워크 –`. 옛 봉(`null`)도 같은 표기.
- **사건 표**(013 §3.5 좌 카드): 열 `티커|상태|횟수|최대 지속|평균 지속|최대 스프레드|평균 스프레드|네트워크|최신`. `네트워크` = 그 심볼의 **가장 최근 사건**의 `netDom`(거래소 전체면 두 거래소 사건을 합친 뒤 최근 것), 없으면 `–`. 정렬은 문자열 오름차순(재클릭 반전).
- **사건 로그**(우 column): 열 `거래소|시작|종료|지속|최대 스프레드|네트워크`. `거래소` = `{국내} · {해외}`(같은 초에 해외 거래소별로 열린 사건을 구분한다). `네트워크` = 그 사건의 김프면 `{netFx} → {netDom}`, 역프면 반대. 없으면 `–`.
- 요약 카드·타임라인은 그대로.
- 접기(rollup)에서 두 문자열은 마지막 값(입출금 4상태와 같은 규칙).

### 3.9 엣지
- 국내 거래소 조회 실패 회차(D 비고 코인 값 `None`): 틱 4상태 `None`·망 없음 → 봉 −1·빈 문자열, 막힌 초 안 셈, 화면 `모름`·`네트워크 –`. 표와 같다.
...
web(수동 또는 순수 함수):
- 사건 표 `네트워크` 열 = 최근 사건 기준·정렬, 사건 로그 `네트워크` 열 방향 표기, 읽기 줄 표기 4가지(둘 다·한쪽·없음·옛 봉).
...
- 사건 표에 `네트워크` 열, 새로 열린 사건 행 클릭 → 로그에 `ERC20 → Ethereum` 형태. 배포 전 사건은 `–`.
