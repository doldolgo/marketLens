...
## 4. 검증
- env 파일(없으면 env 예시 파일에서 만든다)을 둔 채 `WEB_PORT=8080 docker compose --env-file server/.env up -d --build` 하면 다섯 컨테이너가 살아 있다(compose 변수 치환은 셸 env 와 `--env-file` 만 읽으므로 `INFLUX_TOKEN` 을 위해 `server/.env` 를 명시한다).
