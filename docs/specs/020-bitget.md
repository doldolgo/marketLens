...
- 심볼마다 두 채널. **`books15`** — 위 15단계 스냅샷, **200ms 마다** `action:"snapshot"` 으로 통째 온다(update 는 오지 않지만 코드는 `update` 도 받아 둔다 — 전체 깊이 `books` 로 되돌릴 때를 위해). 페이로드 `{"action":…,"arg":{…,"instId":"BTCUSDT"},"data":[{"asks":[["<가격>","<잔량>"],…],"bids":[…],"seq":<정수>,"ts":"<ms 문자열>"}],"ts":<ms>}` — 가격·잔량 문자열, `asks` 오름차순·`bids` 내림차순, `data` 는 원소 1개. **`trade`** — 체결 즉시 `{"action":"snapshot"|"update","arg":{…,"channel":"trade"},"data":[{"ts":"<체결 ms>","price":"…","size":"…","side":"buy|sell","tradeId":"…"},…]}`. 구독 직후 첫 프레임은 **최근 체결 스냅샷**(과거 체결 포함)이고 이후는 새 체결만 온다.
- 왜 `books15`·`trade` 인가: 현물 호가 채널은 `books`(전체)·`books1`·`books5`·`books15` 뿐이다. 처음엔 `books`(전체 깊이, 스냅샷+update)를 로컬 북으로 들었으나 **2026-09-15 개정** — update 마다 북 전체를 파이썬 `sorted()` 로 다시 정렬하는 비용이 EC2 서버 프로세스 CPU 의 47%(py-spy 실측)였고, 표는 체결 규모 $1,000 기준이라 15단계로 충분하다. 메시지 주기는 둘 다 200ms 로 같아 수신량은 변하지 않는다. 바이낸스 20단계보다 5단계 얕다. 체결가는 `ticker`(스냅샷 주기 푸시) 대신 체결이 있을 때만 오는 `trade` 를 쓴다 — 바이낸스 miniTicker·바이빗 publicTrade 와 의미가 같다.
