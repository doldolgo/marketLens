...
- **닫는 조건** 둘 중 먼저 오는 것: ① 60초 경과 ② 버퍼의 비압축 바이트가 32MB 도달. 닫힌 버퍼 = 객체 1개. 두 값은 코드 상수다. "60초 경과" 는 그 버퍼 **첫 줄의 `receivedAt`** 부터 닫기 회차의 현재 시각까지 60,000ms 이상이다. 닫는 주체는 매초 도는 닫기 회차뿐이다 — 기록 함수는 붙이기만 하므로 32MB 는 다음 회차(≤1초 뒤)에 닫힌다. 닫기 회차는 업로드와 독립이라(§3.6) S3 가 느리거나 막혀 있어도 이 주기는 유지된다 — 열린 버퍼는 32MB + 한 회차치(1초 + 직전 회차의 gzip 시간, 32MB 객체 하나에 0.3초 안팎) 유입 이상 자라지 않는다.
...
- 내용: gzip 압축 JSON Lines, 줄 순서 = 수신 순서. 메타데이터 `Content-Type: application/x-ndjson`, `Content-Encoding: gzip`. gzip 압축 레벨은 6(코드 상수) — 32MB 원문에 0.3초 안팎이고, 레벨 9 는 3배 넘게 걸리면서 압축률 이득이 1% 미만이라 닫기 회차의 "매초" 를 지키는 쪽을 택한다. 같은 버퍼를 두 번 직렬화하면 바이트까지 같다(레벨 고정·gzip mtime 0 고정).
- 빈 버퍼는 객체를 만들지 않는다.
...
- **기록되지 않는 경우** (원문이 아카이브에 없는 경로는 이 둘뿐이다): ① UTF-8 로 디코드되지 않는 바이너리 프레임 — 문자열이 없어 수신 경로가 기록 없이 버린다(001·012 의 무효 프레임 카운트에만 남는다). ② 위의 회차 예외로 잃은 닫힌 줄들. 그 밖에 대기열 상한 초과(§3.6)·종료 데드라인 초과(아래)는 닫힌 객체 단위로 버리며 각각 로그를 남긴다.
