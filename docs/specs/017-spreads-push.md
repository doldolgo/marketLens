...
- 하지 않는 것: `GET /spreads` 의 서빙 방식(018 이 Redis 읽기로 바꾼다), 분석 엔드포인트·`/health/collect` 의 푸시화, 호가 원본 공유, 인증, 접속자 수 집계 화면, 티커 기반 김프의 브라우저 계산, 수집 프로세스의 페이로드 압축(수집 → Redis → api 는 로컬 도커망이라 이득이 없고 수집 CPU 만 쓴다 — 압축 없이 게시한다. api → 브라우저 프레임은 허브가 **표 1장당 1회** gzip 한 바이너리다(3.2·3.3) — uvicorn 의 permessage-deflate 는 접속마다 따로 압축해 api CPU 가 접속자 수에 비례하므로 끈다(부하 실측 2026-09-26 §5: 50명에 1코어 포화). 실측(§5) 바뀐 행 35% × 238KB ≈ 83KB/s 라 압축 없이는 폴링(gzip 54KB/s)보다 브라우저 대역폭이 커진다. HTTP 는 기존 GZip 미들웨어)
...
- **접속자와 무관하게 매 틱 만든다**(2026-09-26 결정 — 이전에는 서빙 프로세스가 접속자가 있는 동안 `spreads:want` 를 갱신하고 수집이 그 키를 5초마다 읽어 "원함" 일 때만 만들었다. 첫 접속자가 최대 5~6초 `waiting` 을 보는 것이 문제였다). 표 1장 ≈ 50ms 라 수집 프로세스의 상시 부담은 약 5%p. 서빙 프로세스(3.2)의 `spreads:want` 갱신은 남아 있으나 수집은 읽지 않는다 — 첫 접속자는 `spreads:latest` 로 즉시 snapshot 을 받는다.
...
- 새 표가 오면 diff 를 1회 만들고 **gzip(레벨 6) 도 1회** 해서, 접속자 전원에게 **같은 바이트**를 바이너리 프레임으로 보낸다. 접속자별 직렬화·압축은 없다 — 접속자당 남는 비용은 소켓 쓰기뿐이다. `heartbeat`·`waiting` 도 미리 압축해 둔 같은 형식이라 브라우저는 프레임 종류를 분기하지 않는다. 그래서 `api` 컨테이너는 uvicorn 을 `--ws-per-message-deflate false` 로 띄운다 — 켜 두면 이미 압축한 바이트를 접속마다 또 압축한다(로컬 개발의 uvicorn 은 플래그 없이도 동작하되 이중 압축).
...
### 3.4 FE — 구독
- 셸이 공유 피드를 만든 직후 **연결 하나**를 연다(폴링이 살던 자리). 프레임은 브라우저 내장 `DecompressionStream("gzip")` 으로 푼다(Chrome 80·Firefox 113·Safari 16.4 이상, 폴백 없음). 해제가 비동기라 **도착 순서대로 직렬 처리**한다 — delta 순서가 바뀌면 병합·삭제가 틀어진다. 주소는 `API_BASE` 가 상대경로(`/api`)이므로 **페이지 origin 기준 절대 URL** 로 만든 뒤 스킴만 `http→ws`·`https→wss` 로 바꾼다 — 결과는 `ws(s)://<host>/api/ws/spreads`.
...
- 끊기면 1→2→4→…→30초 백오프로 재연결한다(서버 메시지를 하나라도 받으면 백오프는 1초로 돌아간다). 재연결 대기 중에도 직전 표를 지우지 않는다. **snapshot 을 아직 한 번도 못 받았으면 상한은 3초**(2026-09-26 — 배포 직후 새로고침에서 첫 표까지 30초를 기다리지 않게).
...
- 수집: `spreads:want` 가 없어도 매 틱 표를 만들어 채널·키에 쓴다 / 채널 페이로드는 **같은 틱 안에서** 만든 `GET /spreads` 응답과 JSON 이 같다(시각 필드 `fetchedAt`·`dataReceivedAt`·`age` 는 같은 틱에서 비교)
- 수집: Redis 불달이면 게시만 건너뛰고 틱 인계는 계속
- diff: `age` 만 바뀐 행은 안 실린다 / `status` 가 바뀐 행은 실린다 / 사라진 행은 `removed` / 바뀐 행 0개여도 delta 1개
- diff: 접속자 3명에게 간 바이트가 동일하다 / 접속자 0명일 때 채널 메시지는 파싱되지 않는다 / 프레임을 gzip 으로 풀면 정확히 메시지 JSON 이다(heartbeat·waiting 포함)
- `/ws/spreads`: 접속 → snapshot, 표 없음 → waiting → 첫 표에 snapshot, 1초 무전송 → heartbeat, 클라이언트 메시지는 무시
- 서빙: 첫 접속자가 `spreads:latest` 로 즉시 snapshot 을 받는다 / 접속자가 있으면 `spreads:want` 가 5초마다 갱신된다 / 0명이면 갱신하지 않는다(수집은 이 키를 읽지 않는다)
