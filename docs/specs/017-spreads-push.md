...
### 3.1 수집 프로세스 — 표 게시
- 규모는 서버 상수 **`1000`**(USD) — `GET /spreads` 기본값과 같다(003 §3.2-0 의 기본값을 이 값으로 바꾼다. 범위 규칙 `1 ≤ notional ≤ 10_000_000` 은 유지).
...
- 남은 빚:
  - `camelize_json`(dict 재귀 재조립)이 표 1장 비용의 절반(로컬 7ms/16ms) — pydantic alias 직렬화로 바꾸면 모든 라우터·게시가 같이 빨라진다(003·004·005 공통, 별도 chore).
  - 표 생성은 틱 루프 안 동기(EC2 약 50ms 추정) — 배포 뒤 "표 생성 … 초과" 경고가 보이면 표 계산의 규모 무관 부분 분리를 검토한다("await 없음" 원칙은 유지).
