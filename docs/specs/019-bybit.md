...
- 심볼마다 두 토픽. **`orderbook.200.<SYMBOL>`** — 구독 직후 `type:"snapshot"`(전체 북), 이후 바뀐 단계만 `type:"delta"` 로 최대 100ms 마다. **행 발행 제한(2026-09-15 개정)**: 델타는 받는 즉시 전부 로컬 북에 반영하되(바뀐 단계만 오므로 하나라도 빠지면 북이 어긋난다), 북을 정렬해 행으로 내보내는 것은 **심볼당 500ms 에 최대 1번**이다 — 마지막 발행 뒤 500ms 가 지난 심볼의 델타는 바로 내보내고, 그 안에 온 델타는 북에만 쌓아 두었다가 샤드별 500ms 주기 태스크가 묶어서 내보낸다(델타가 끊긴 심볼도 마지막 델타 뒤 500ms 안에 나온다). `snapshot` 은 간격과 무관하게 즉시. 행의 호가 시각·`updated_at` 은 그 북에 마지막으로 반영된 프레임의 `ts`·수신 시각. 이유: EC2 py-spy 에서 델타마다 200단계 정렬·행 재생성이 수집기 CPU 22% 였는데 표(ticks)는 LiveStore 를 1초에 1번만 읽는다. 페이로드 `{"topic":…,"type":…,"ts":<ms>,"data":{"s":"BTCUSDT","b":[["<가격>","<잔량>"],…],"a":[…],"u":<update id>,"seq":…},"cts":<ms>}` — 가격·잔량 문자열, `b` 내림차순·`a` 오름차순. **`publicTrade.<SYMBOL>`** — 체결 즉시 `{"topic":…,"type":"snapshot","ts":<ms>,"data":[{"T":<체결 ms>,"s":…,"S":"Buy|Sell","v":"<수량>","p":"<가격>","i":…},…]}`, 한 프레임에 최대 1,024건.
