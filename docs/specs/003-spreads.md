...
- 가격 기준: `현재가`(기본) / `슬리피지 반영`. 슬리피지 반영이면 체결 규모 `$10k`(기본)·`$50k`·`$100k`·`$500k` 를 고른다. 행마다 `slip = s(liqFx) + s(liqDom)`, `s(liq) = min(6, 100 × (규모 / max(liq, 1))^0.85)` — 규모 대비 최우선 호가 유동성이 작을수록 커지고 한쪽 상한 6%. 행의 fwd·rev 에서 `slip` 을 빼고 소수 2자리로 반올림한 값이 그 뒤의 모든 판단(최대 행 선택·강조·임계 필터·정렬·표시)에 쓰인다. 김프 셀의 값 왼쪽에 `슬 −N.NN%p`(차감폭) 배지를 보인다. 현재가 기준이면 배지 없음.
