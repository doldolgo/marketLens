...
## 1. 목적
서버 메모리의 실시간(WebSocket) 시세만으로 **전 (국내 거래소 × 해외 거래소 × 코인) 페어의 김프/역프 표**를 `GET /spreads` 로 낸다. 표의 김프/역프는 체결 규모 **$1,000**(서버 상수 — 017)로 **호가창을 실제로 걸어** 얻은 슬리피지 차감 후 순값이다 — 추정식이 아니라 그 규모를 밀었을 때 실제로 남는 값이다.
...
- BE 엔드포인트 `GET /spreads`(HTTP 서빙은 018 — 이 스펙은 표 **계산**), `POST /refresh`.
- FE 스프레드 탭(표 수신은 017 의 WebSocket 구독).
...
### 3.2 표 계산 (017 게시기가 매초 호출한다)
0. 규모는 서버 상수 `1000`(USD, 017). 표의 모든 행이 이 규모로 호가를 걷는다. 표 최상위에 쓰인 값을 `notional` 로 싣는다 — 소비자가 `slipFwd` 가 어느 규모의 값인지 표만 보고 알게 하기 위해서다.
1. 기준 거래소(`upbit`)의 환율이 없거나 ask/bid 가 0 이하 → 표를 만들지 않는다(018 §3.2).
2. 스냅샷을 호가통화로 나눈다. `KRW` → 국내, `USDT` → 해외, 그 외 무시. 어느 한쪽이 비면 표를 만들지 않는다(018 §3.2). 수집 전 빈 메모리는 1 또는 2 에 걸리며 구분하지 않는다.
...
4. 행 하나의 규칙. 이 행 국내 거래소 USDT 시세의 ask 를 `rate_ask`, bid 를 `rate_bid` 라 한다 — **계산에만 쓰고 응답에는 싣지 않는다**(소비자는 최상위 `rate` 와 행의 `krw` 를 쓴다).
   ...
   - `usd` = 해외 스냅샷의 마지막 체결가. `spark` = fwd 추이(009 가 채운다)를 **소수 3자리로 반올림해** 싣는다 — 490행 × 30개를 1초마다 보내므로 배정밀도 그대로면 응답이 gzip 106KB 다(3자리면 60KB 안팎). 0.001%p 는 화면 스파크라인 눈금보다 촘촘하고, 009 의 버퍼와 Influx 에는 원값이 남는다.
...
- `usd` 해외 마지막 체결가(USDT).
- `spark` 프리미엄 추이(009 가 채운다) — 응답 값은 소수 3자리다.
- `status` `ok`·`stale`·`fail` 중 하나. `age` 두 거래소 스트림 중 오래된 쪽의 마지막 수신 경과 초(행 자체가 300초 이상 안 바뀌었으면 그 행의 경과 초).
...
- 표는 **017 의 `/ws/spreads` 구독**으로 받는다(snapshot·delta·재연결 규칙은 017 §3.4 — 폴링 fallback 없음). `GET /spreads` 는 브라우저가 부르지 않는 curl·진단용이고 api 가 Redis 에서 답한다(018). 행의 `dom`/`fx` 는 표시명(`upbit→업비트`, `bithumb→빗썸`, `binance→Binance`, `bybit→Bybit`, `bitget→Bitget`, 모르는 id 는 그대로)으로 바꾼 뒤 공유 피드에 적용하고 화면을 갱신한다. **수신 실패는 직전 데이터를 유지**한다.
...
- 체결 규모: **$1,000 고정** — 선택지가 없다(017). **FE 는 슬리피지를 계산하지 않는다** — 응답의 `fwd`·`rev` 가 이미 순값이라 그 값이 그대로 모든 판단(최대 행 선택·강조·임계 필터·정렬·표시)에 쓰인다.
...
- 필터바 2행: "기준 보기"(김프/역프) · 세로선 · "체결 규모" 라벨 + `$1,000` 안내 문구(차감이 항상 적용되므로 항상 보인다) · 세로선 · "비교 해외 거래소" `모두` 체크박스 · 세로선 · 002 §3.6 의 해외 거래소 6곳 체크박스(기본 전부 체크). `모두` 는 6곳 전체 토글이다. 수집 대상이 아닌 거래소는 행이 없으므로 체크를 풀어도 표가 변하지 않는다.
...
- 규모 미지정이면 1000 이 쓰이고 표 최상위에 그 값이 실린다.
...
- 규모 500000 으로 계산하면 같은 행의 `fwd` 가 기본 규모보다 작거나 같고 `slipFwd` 가 크거나 같다(계산 함수로만 — HTTP 는 $1,000 표뿐, 018).
...
- 추측한 지점 (묻지 않고 정한 것 — 전부 본문에 반영):
  ...
  - **체결 규모는 $1,000 고정**(017) — 셸 상태·URL 키가 없다.
