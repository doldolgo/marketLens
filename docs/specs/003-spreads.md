...
4. 행 하나의 규칙. 이 행 국내 거래소 USDT 시세의 ask 를 `rate_ask`, bid 를 `rate_bid` 라 한다 — **계산에만 쓰고 응답에는 싣지 않는다**(소비자는 최상위 `rate` 와 행의 `krw` 를 쓴다).
   ...
   - `usd` = 해외 스냅샷의 마지막 체결가. `spark` = fwd 추이(009 가 채운다) — 값은 **소수 3자리**다(009 가 링버퍼에 넣는 순간 반올림한다 — 표를 만들 때마다 전 행을 다시 반올림하지 않는다). 490행 × 30개를 1초마다 보내므로 배정밀도 그대로면 응답이 gzip 106KB 다(3자리면 60KB 안팎). 0.001%p 는 화면 스파크라인 눈금보다 촘촘하고, Influx 에는 원값이 남는다.
...
- `usd` 해외 마지막 체결가(USDT).
- `spark` 프리미엄 추이(009 가 채운다) — 값은 소수 3자리다(009 가 넣을 때 반올림).
