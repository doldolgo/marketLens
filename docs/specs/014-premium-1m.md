...
- 계층마다 **마지막으로 접은 창**을 메모리에 든다. 회차마다 그 다음 창부터 **끝이 지금 이전인 창**까지 차례로 접어 쓴다(같은 키 덮어쓰기라 재시도 안전). 한 회차에 계층당 **최대 12창**, 나머지는 다음 회차 — 재기동 따라잡기가 아래 계층을 한꺼번에 읽어 Influx 를 누르지 않게.
- **재기동 따라잡기**: 기동 시 계층마다 위 버킷의 가장 늦은 점을 "마지막으로 접은 창" 으로 삼는다(3초 상한, 실패하면 지금 창으로 시작하고 경고 1줄). 위 버킷이 비어 있으면(첫 배포) 아래 버킷의 가장 오래된 점이 든 창부터. 아래 계층의 보관 기간을 넘긴 빈틈은 메울 수 없다 — 7일 안에 다시 켜지면 1m 부터 전 계층이 빈틈 없이 복구된다.
...
- 롤업: 1m 5개(시가 a·종가 e·고 max·저 min·samples 합·blocked 합) → 5m 1점, 마지막 점 값이 가격·입출금; 1m 이 3개뿐이면 그것으로 접는다, 0개면 5m 점 없음; 순서 = 1m 쓰기 → 5m → 1h → 4h → 1d(1h 는 같은 회차에 쓴 5m 을 읽는다); 한 회차 계층당 12창까지, 13번째는 다음 회차; 끝이 지금 이후인 창은 접지 않는다
- 따라잡기: 기동 시 위 버킷 마지막 점 다음 창부터; 위 버킷이 비면 아래 버킷 가장 오래된 점의 창부터; 조회 실패 시 지금 창부터 + 경고
