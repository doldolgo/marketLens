...
`rate` 는 그 국내 거래소의 USDT 중간값(원). 입출금 4상태는 006 §3.7 판정값(024 — spreads 행과 같은 함수)이고 3상태 그대로(`None` 유지). 망 이름 2개는 판정에 쓴 표시명이며 모르면 없음. Redis 레코드(009 §3.3)·`premium` 점에는 넣지 않는다.

### 3.3 1분 집계 규칙
- 분 = `ts // 60 * 60`. 조합 `(dom, fx, base)` 마다 **그 분에 행이 있던 틱만** 모은다.
- `fwd`·`rev` 각각 OHLC: 시가 = 그 분 첫 행, 고·저 = 최대·최소, 종가 = 마지막 행. 가격 3개·입출금 4상태·망 이름 2개 = 마지막 행 값(망 없음은 빈 문자열). `samples` = 그 분의 행 수(1~60).
...
- measurement 는 다섯 버킷 모두 `candle`. tag `dom`·`fx`·`base`, time = 창 시작(초). field `fwd_o fwd_h fwd_l fwd_c rev_o rev_h rev_l rev_c`(float %, 원값)·`krw`(float, 국내 종가 원)·`usdt`(float, 해외 종가)·`rate`(float, USDT 중간값 원)·`dom_dep dom_wd fx_dep fx_wd`(int: 1 가능·0 불가·−1 모름)·`blocked_fwd_sec blocked_rev_sec`(int, 창 길이 초 이하)·`samples`(int, 그 창에 든 틱 수)·`net_dom net_fx`(string, 빈 문자열 = 없음) — 20 필드. **읽을 때** 문자열 2필드는 선택이다: 없거나 빈 문자열이면 `null` 로 읽는다(배포 전 18 필드 점을 버리지 않기 위해 — 수치 18 필드가 하나라도 없으면 반쪽 점으로 버린다). 유일키 = (버킷, dom, fx, base, 창 시작). 1m 하루 ≈ 70만 점, 계층 전체 합쳐도 초 단위 `premium` 의 1/40 이하.
...
- 접기 규칙: 시가 = 첫 아래 점의 시가, 고·저 = 최대·최소, 종가 = 마지막 점의 종가(fwd·rev 각각). 가격 3개·입출금 4상태·망 이름 2개 = 마지막 점 값. `blocked_*_sec`·`samples` = 합. 아래 점이 일부만 있으면 있는 것으로 접고, 하나도 없으면 위 점도 없다.
...
```json
{
  ...
  "candles": [
    {"ts": 1788739200, "open": 0.62, "high": 0.71, "low": 0.58, "close": 0.66,
     ...
     "blockedSec": 0, "samples": 60, "netDom": "Ethereum", "netFx": "ERC20"}
  ]
}
```
...
- `netDom`·`netFx` = 저장 문자열, 빈 문자열·없음은 `null`. 방향과 무관하게 같은 값(국내 망 하나를 기준으로 판정하므로 김프·역프가 같은 망을 본다).
...
- 데이터 = 선택된 (국내 × 해외) 쌍마다 `GET /history/candles` 를 **청크** 단위로 부른다. 청크 = **360창**(`INITIAL_BARS` 와 같은 값 — 1m 6시간·5m 30시간·1h 15일·4h 60일·1d 360일), KST 기준 청크 길이의 배수로 정렬(`(ts + 32400) // 청크 * 청크 − 32400` — 청크가 창 길이의 배수라 창이 청크에 걸치지 않는다). 서버 상한(§3.6)을 그대로 청크로 쓰면 첫 로드가 필요한 양의 4배(1m 하루 청크 ≈500KB × 쌍 6개, 새벽엔 어제 청크까지 12건)를 받아 느렸다. 청크는 `(dom, fx, base, dir, res, 청크 시작)` 키로 메모리에 들고 있어 심볼·방향·쌍·봉을 바꿔도 **없는 청크만** 새로 부른다. 처음엔 `INITIAL_BARS × 봉 초`를 덮는 청크 수, 왼쪽으로 끌면 청크 하나 추가, **과거 상한 = 그 계층의 보관 기간**(1d 는 상한 없음). 요청은 디바운스 400ms·직전 요청 취소(013 §3.5 방식). 청크가 상한보다 짧아 400 은 나올 수 없다.
- **최신 청크는 60초마다 다시 부른다**(`HISTORY_EVENTS_POLL_MS` 와 같은 주기의 별도 상수) — 창이 닫힐 때마다 새 봉이 붙는다. 과거 청크는 다시 부르지 않는다. 숨김 탭에서도 돈다(013 과 같은 이유).
- `Candle1m` 의 `depositOk`·`withdrawOk` 는 `boolean | null`. 입출금 띠: 둘 다 `true` 면 열림 색, 어느 하나 `false` 면 막힘 색(`blockedSec ≥ 창 길이` 진하게, 아니면 옅게 — 시안 그대로), **`null` 이 섞이고 `false` 가 없으면 회색(모름)**. 띠 라벨은 방향 경로로: 김프 `{해외} 출금 → {국내} 입금`, 역프 `{국내} 출금 → {해외} 입금`. 접을 때 입출금·망 이름은 마지막 값, `blockedSec` 은 합(시안 그대로). **읽기 줄**의 거래소별 입금·출금 칸 옆에 망을 한 칸 더 보여준다 — 김프 `{netFx} → {netDom}`, 역프 `{netDom} → {netFx}`(경로 방향대로), 한쪽만 있으면 있는 쪽만(`– → Ethereum`), 둘 다 없으면 `망 –`(옛 봉도 같다).
- 해외 거래소 선택지는 서버가 수집하는 `binance`·`bybit`·`bitget`(019·020) — 서버에 없는 거래소를 고를 수 있으면 빈 차트만 나온다. **코인이 바뀔 때(표 클릭·검색·URL 복원) 켜진 해외 카드 중 그 코인이 있는 거래소가 하나도 없으면, 스프레드 피드에서 그 코인이 있는 해외 거래소 전부로 카드를 바꾼다**(예: CUDIS 는 빗썸×Bitget 뿐이라 Bitget 카드로). 하나라도 있으면 사용자 선택을 그대로 둔다. 피드가 아직 없으면 판단하지 않는다. 그래도 빈 카드가 되면 판의 문구는 `기간 내 기록 없음` 대신 `{코인} 은 {있는 거래소} 에만 있습니다`.
...
#   요청 res=1m 청크 2개(6시간 단위), "5분" 클릭 → res=5m&start=…&end=start+108000 요청
...
- **2026-09-26 청크 축소(§3.7 함께 고침)**: 청크를 서버 상한(1,440창) → 360창으로. 실서버 측정: 1m 하루 청크 = 1,440봉·원본 503KB·gzip 68KB·건당 0.3~0.5초, 첫 로드는 쌍 6개 × 청크 1~2개 = 6~12건이라 느렸다. 360창이면 첫 로드 2청크 × 6쌍 = 12건이되 건당 4분의 1, 최신 청크 60초 재조회도 4분의 1. 과거로 끌 때 요청은 4배 잦아지지만 총량은 같다. 서버 변경 없음.
- 남은 빚:
