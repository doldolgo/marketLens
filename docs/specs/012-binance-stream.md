...
## 2. 범위
- 만드는 것: `core/connectors/` 의 바이낸스 스트림 커넥터(샤드 3개·구독 재조정·디코딩·핑·재연결·정체 판정), 심볼 목록 REST 조회, 001 스트림 상태 계약의 바이낸스 구현, 테스트. 라이브러리 `websockets`.
- 하지 않는 것: REST 시세 호출(없다). diff depth 와 로컬 북 재구성(§3.2). 깊이를 쓰는 계산(003·004). HTTP 계약 변경.
...
- 구독 대상 = 001 의 마켓 우주 심볼. 우주가 바뀌면(60초 재조정 주기 또는 `/refresh`) 새 심볼은 SUBSCRIBE, 빠진 심볼은 UNSUBSCRIBE 하고 그 행을 메모리에서 지운다. 기동 직후 우주가 비어 있으면 구독이 없다.
...
- 구독은 연결 후 `{"method":"SUBSCRIBE","params":["btcusdt@depth20","btcusdt@miniTicker",…],"id":<int>}` 로 한다. 한 메시지의 `params` 는 **100 개 이하**, 제어 메시지는 **초당 4개 이하**로 보낸다(한도: 연결당 수신 메시지 5개/초 — PING·PONG 포함). 응답 `{"result":null,"id":…}`.
- 연결 시도는 IP 당 5분에 300회 한도 — 재연결은 지수 백오프(1·2·4…30초 상한), **구독까지 성공하면 1초로 리셋**. 서버는 20초마다 ping 프레임을 보내고 1분 안에 pong 이 없으면 끊는다 — 라이브러리의 자동 pong 을 쓴다. 연결은 24시간에 한 번 서버가 끊으므로 재연결이 정상 경로다.
...
- miniTicker 메시지 → `price = c`, `price_timestamp = E`. 행이 없으면 보류했다가 depth20 이 오면 함께 싣는다. 체결가가 없으면 mid.
- 캐시 키는 원본 심볼 대문자(`BTCUSDT`) — 스트림 이름(`btcusdt@…`)과 `native_symbol` 이 같은 값으로 이어져 base↔symbol 변환이 한 곳(`baseAsset`)에만 있다.
- 입출금 3필드 물려받기·우주 밖 버리기는 001 §3.5.

### 3.5 정체 판정 (매 틱, 샤드 단위)
- 샤드마다 시계를 따로 둔다: `last_message_at`(그 샤드가 마지막으로 시세 프레임을 받은 시각), 구독 수, 구독 시작 시각. 구독이 0 인 샤드는 판정 대상이 아니다.
- 매 틱 바이낸스는 **셋 다 정상**이어야 성공이다. 어느 샤드든 미연결이면 그 샤드 `last_error.kind` 로 실패, 연결됐는데 **30초** 무수신이면 `stale_stream`. 여러 샤드가 동시에 나쁘면 **가장 오래 조용한** 샤드를 고른다(동률이면 작은 번호). 실패 `message` 는 샤드를 말한다: `"바이낸스 스트림 정체: 샤드 2 (구독 7종목) 30초 이상 무수신"`. `url` = WS URL, `status_code` 는 핸드셰이크 실패일 때만.
- 정체 중에도 행은 그대로 남는다(001 — 행은 메시지로만 바뀐다). 스트림이 돌아오면 다음 틱이 성공으로 기록돼 구간이 닫힌다.
- 001 스트림 상태 계약의 바이낸스 값: `connected` = 샤드 셋 다 연결, `last_message_at` = 샤드 중 최신, `last_error` = 판정에 쓴 샤드의 것, `subscribed` = 합.

### 3.6 장애 격리
- 한 번도 못 붙어도 앱은 뜬다. 경고 로그 1줄, 백오프 재시도, 바이낸스 행은 비어 003 이 404 또는 행 없음으로 동작한다.
- 종료: 샤드 태스크 취소 후 소켓 3개를 **동시에** 닫고 합계 2초 상한(상대가 close 프레임을 안 주면 하나당 20초를 기다려 `docker stop` 10초를 넘긴다). 남은 TCP 는 OS 가 정리한다.
...
- SUBSCRIBE 한 메시지 ≤ 100 스트림, 초당 ≤ 4 메시지. 재조정: 새 심볼 구독·빠진 심볼 해지·행 삭제.
