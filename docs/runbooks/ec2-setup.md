...
5-2. IAM 역할 (010 — 현 인스턴스엔 역할 `marketlens-s3-snapshot` 부착·적재 확인 완료(2026-09-03), hop limit 도 이미 2. 재구축 시): 정책은 최소 2개 — 버킷 `raw/*` 접두사에 `s3:PutObject`, 버킷에 `s3:ListBucket`(HeadBucket 용). 원문 아카이브는 하루 10~20GB(추정)가 쌓이므로 보존 기간을 정해 버킷 lifecycle 로 건다 — 코드는 관여하지 않는다. 역할을 EC2 인스턴스에 부착한 뒤, 컨테이너에서 인스턴스 메타데이터(IMDSv2)에 닿도록 hop limit 을 2 로 올린다(기본 1 은 docker 브리지 안에서 안 닿는다):
