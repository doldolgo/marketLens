...
5-2. IAM 역할 (010): 정책은 최소 2개 — 버킷 `spreads/*` 접두사에 `s3:PutObject`, 버킷에 `s3:ListBucket`(HeadBucket 용). 역할을 EC2 인스턴스에 부착한 뒤, 컨테이너에서 인스턴스 메타데이터(IMDSv2)에 닿도록 hop limit 을 2 로 올린다(기본 1 은 docker 브리지 안에서 안 닿는다):
