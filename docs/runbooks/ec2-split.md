...
| 그룹 | 인바운드 규칙 |
|---|---|
| collect | TCP 8000 ← `marketlens-serve` 그룹 / TCP 22 ← 내 IP |
| data | TCP 6379 ← `marketlens-collect`·`marketlens-serve` 그룹 / TCP 8086 ← 같은 두 그룹 / TCP 22 ← 내 IP |
| serve | TCP 80 ← 0.0.0.0/0 / TCP 22 ← 내 IP |

소스에 CIDR 대신 **보안그룹 ID** 를 넣는다 — 사설 IP 가 바뀌어도 규칙이 산다.
...
| 박스 | 종류 | 루트 | 보안그룹 | 추가 |
|---|---|---|---|---|
| collect | c7g.medium | 16GB | marketlens-collect | IAM 프로파일 `marketlens-s3-snapshot`, 탄력 IP 새로 1개 |
...
생성 뒤 세 박스에서 `ec2-setup.md` 1번(도커·compose 플러그인, `ubuntu` 를 `docker` 그룹에)과 3번(`git clone … ~/marketlens`)을 한다.
...
```
- 확인: 브라우저 `http://3.34.104.16/` 스프레드 표 매초 갱신, 기록 탭 차트, 수집 상태 탭 5거래소 ok. `ssh team` 은 이제 serve 로 붙는다(`~/.ssh/config` 의 `team` 은 그대로 serve 를 뜻하게 된다 — collect·data 별칭을 추가한다).
