...
```
로그에 `바이낸스 깊이 샤드 N 구독 완료` 3줄(샤드 0·1·2)이면 정상. 깊이는 HTTP 로 노출되지 않으므로 값 확인은 로그로 한다 — 스트림이 안 붙어도 `/spreads` 는 최우선 1단계로 정상 동작하고, 구독이 있는 샤드 하나라도 30초 무수신이면 `/health/collect` 의 바이낸스가 `stale_stream` 구간을 보인다(구간 message 에 그 샤드 번호와 구독 수).
