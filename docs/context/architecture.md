...
## 런타임 구성
- **server/**: Python 3.12, FastAPI, httpx, websockets, redis(asyncio), influxdb-client, boto3, pydantic v2, pydantic-settings. 로컬 포트 8000. 상시 태스크: 업비트·빗썸 스트림 각 1, 바이낸스 샤드 3, 틱 루프(1초), Redis 인계 큐, flusher(60초), 원문 업로드 루프, 입출금 조회(60초).
...
- **tick-store (009)**: `core/redis_stream.py`(`RedisTickStream` — redis 를 import 하는 유일한 곳. `ticks` XADD/XRANGE 페이지/XDEL, 재시도 없음, 연결 2초·명령 5초), `core/tick_store.py`(`encode_tick`/`decode_tick` gzip JSON, `TickRelay` — `handoff` 구현: spark 갱신·게시 → 600 큐 → 보내기 태스크가 순서대로 XADD, 실패 틱은 버림; `Flusher` — 60초마다 전량 읽기 → 5,000점 배치 → 성공 시에만 XDEL, 연속 실패 수·잘림 감지), `core/spark.py`(`SparkBuffer` 1분 버킷 last 30개 링버퍼, `restore_spark` 기동 복원 10초 상한), `core/influx.py` 의 `query_spark`(aggregateWindow 1m last). `INFLUX_TOKEN` 없으면 flusher 를 띄우지 않는다. 테스트는 `server/tests/test_tick_store.py`·`test_spark.py`·`test_tick_store_history.py`(fakeredis + `tests/conftest.py` 의 `FakeInflux`).
