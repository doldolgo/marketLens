...
- **원문은 전부 S3 에 남긴다(010).** 거래소가 준 모든 WebSocket 프레임·REST 응답 본문을 해석 전에 원문 싱크에 넘기고, 거래소별로 60초(또는 32MB) 객체로 S3 `raw/` 에 쌓는다. 가공값은 S3 에 쓰지 않는다 — 쓰는 필드가 바뀌어도 재수집 없이 재생하기 위한 저장소다.
...
## 런타임 구성
- **server/**: Python 3.12, FastAPI, httpx, websockets, redis(asyncio), influxdb-client, boto3, pydantic v2, pydantic-settings. 로컬 포트 8000. 상시 태스크: 업비트·빗썸 스트림 각 1, 바이낸스 샤드 3, 틱 루프(1초), Redis 인계 큐, flusher(60초), 원문 닫기 회차(1초, 업로드는 데몬 워커 스레드 1개), 입출금 조회(60초).
...
[업비트 WS] [빗썸 WS] [바이낸스 WS 3샤드]   [REST: 마켓 목록 10분 · 입출금 60초]
      │           │            │                          │
      └───── 모든 프레임·응답 본문 → 원문 싱크 ──▶ S3 raw/ (거래소별 60초·32MB 객체)  ─ 010
      │           │            │
...
- **history (005)**: `core/influx.py`, `features/history/`(이력 조회 API), `scripts/backfill.py`, `docker-compose.dev.yml`. web 기록 탭은 mock 데이터를 사용한다. Influx 쓰기는 009 의 flusher 가 한다.
...
- **raw-archive (010)**: `core/raw_archive.py`(`RawArchive` — 001 의 `record` 계약 구현: 줄 조립(`format_line` — JSON 원문은 바이트 그대로, 아니면 문자열)·거래소별 버퍼·닫힌 순서 단일 FIFO 대기열(압축 후 256MB, `threading.Condition` 아래)·매초 닫기 회차(태스크 — 60초·32MB 에 닫아 스레드에서 gzip·넣기, 업로드는 기다리지 않는다)·업로드 워커(데몬 스레드 1개 — 머리 `PutObject`, 실패는 머리에 두고 1초 뒤 재시도)·종료 5초 비우기(못 올린 것은 버림, 워커는 join 하지 않는다), 키 `object_key`), `core/s3.py`(`S3Uploader` — boto3 를 import 하는 유일한 곳, `put`·`head_bucket`, 로그 없음·실패는 예외). 수신 경로와 분리한 이유: 기록 함수는 메모리에 줄을 붙일 뿐이라 S3 의 지연·실패가 커넥터의 한 줄도 막지 않고, 닫기 주기와 업로드를 분리해 S3 가 막혀도 열린 버퍼가 32MB + 한 회차치 유입 이상 자라지 않는다(gzip 은 레벨 6). 원문이 아카이브에 없는 경로는 디코드 불가 바이너리 프레임과 회차 예외로 잃은 닫힌 줄 둘뿐이다(010 §3.6). `main.py` 는 `S3_BUCKET` 이 있을 때만 배선하고(클라이언트 생성·HeadBucket 실패는 각각 에러 1줄, 생성 실패면 비활성) 스트림이 닫힌 뒤 `aclose` 한다. `config.py` 는 빈 `S3_REGION` 을 기본 리전으로 정규화한다. 테스트는 `server/tests/test_raw_archive.py`(`put(key, body)` fake, 워커 결과는 폴링).
