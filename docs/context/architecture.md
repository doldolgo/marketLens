...
- **raw-archive (010)**: `core/raw_archive.py`(`RawArchive` — 001 의 `record` 계약 구현: 줄 조립(`format_line` — JSON 원문은 바이트 그대로, 아니면 문자열)·거래소별 버퍼·닫힌 순서 단일 FIFO 대기열(압축 후 256MB)·매초 루프(60초·32MB 에 닫아 스레드에서 gzip·`PutObject`, 실패는 머리에 두고 재시도)·종료 5초 비우기, 키 `object_key`), `core/s3.py`(`S3Uploader` — boto3 를 import 하는 유일한 곳, `put`·`head_bucket`). 수신 경로와 분리한 이유: 기록 함수는 메모리에 줄을 붙일 뿐이라 S3 의 지연·실패가 커넥터의 한 줄도 막지 않는다. `main.py` 는 `S3_BUCKET` 이 있을 때만 배선하고 스트림이 닫힌 뒤 `aclose` 한다. 테스트는 `server/tests/test_raw_archive.py`(`put(key, body)` fake).
