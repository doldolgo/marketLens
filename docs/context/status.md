...
| wallet-status | 업비트(JWT)·빗썸(public)·바이낸스(HMAC)·바이빗(HMAC 헤더) 조회를 틱 루프가 60초마다 병렬 실행·사이 틱은 캐시, 실패 회차는 그 거래소 전 행 `unknown`·`dwFailed`·`/refresh` 경고, 응답 본문은 010 원문 싱크로, `/spreads` 는 국내 망 기준 판정으로 5필드 | - | 표시는 spreads 탭이 담당. 실키 3-true·`netDom` 채움은 EC2 확인 대기(업비트는 허용 IP 필요) |
| deploy | Dockerfile·compose 5컨테이너(server·api·web·influxdb·redis, 호스트 노출은 web 하나 — api 는 `ROLE=api` 로 Influx 조회 + `/ws/spreads`, 016·017)·CI(server·web 무필터)·deploy 워크플로(env 가드 → 미러 동기화 → `up -d --build` → prune)·설정 계약 테스트 `tests/test_deploy.py` | nginx 서빙(:${WEB_PORT}, `/api/` 접두 제거·SPA fallback·index no-store·assets immutable, `/api/history/{premium,streaks,candles}`·`/api/ws/`(WebSocket 업그레이드) 는 api 로 분기) | 로컬 4컨테이너 검증 완료(2026-09-06), 5컨테이너·api 분기·`stop api` 격리 로컬 검증 완료(2026-09-14, :8090). EC2 공존·PR check·자동 배포·행이 있는 상태의 Redis 격리·Influx 첫 점은 GitHub 권한·EC2 대기 |
...
## 알려진 빚
- (019) 원문 아카이브의 바이빗 호가는 델타 프레임을 분당 마지막 1건으로 표본화한 것이라 북을 재생할 수 없다 — 재생이 필요해지면 스냅샷을 따로 남기는 별도 스펙.
- (019) 004 analysis 는 `/orderbook/bybit` 만 열려 있고 나머지 5개 API 의 해외는 바이낸스 고정 — `fx` 선택은 후속.
