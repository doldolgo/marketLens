...
| history | Influx 클라이언트(`core/influx.py`)·`/history/premium`·`/history/streaks`·`/history/streaks/bulk`(Influx 만 읽음 — 불달·토큰 없음이면 503, 메모리 조회는 영향 없음, `start` 없으면 최근 7일 창)·백필 스크립트(upbit×binance 캔들, 앞뒤 빈 구간만) — `premium`·`dw_fail` 쓰기는 009 flusher | 기록 탭 사건 로그 = `/history/streaks` 실데이터(선택 심볼·필터 변경 시 1회 조회, 기간 1주/1달, 스프레드 행 클릭 피벗·초기 BTC) | 티커별 통계 표·요약·타임라인·구간 차트는 후속(1분 롤업과 함께). dev compose 위 첫 점·백필 실행·bulk 100코인 초과는 EC2 확인 대기 |
