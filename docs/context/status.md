...
| deploy | Dockerfile·compose 5컨테이너(server·api·web·influxdb·redis, 호스트 노출은 web 하나 — api 는 `ROLE=api` 로 Influx 조회 + `/ws/spreads` + `GET /spreads`(Redis 키), 016·017·018)·CI(server·web 무필터)·deploy 워크플로(env 가드 → 미러 동기화 → `up -d --build` → prune)·설정 계약 테스트 `tests/test_deploy.py` | nginx 서빙(:${WEB_PORT}, `/api/` 접두 제거·SPA fallback·index no-store·assets immutable, `/api/history/{premium,streaks,candles}`·`/api/ws/`(WebSocket 업그레이드)·`/api/spreads`(정확 일치, 018) 는 api 로 분기) | 로컬 4컨테이너 검증 완료(2026-09-06), 5컨테이너·api 분기·`stop api` 격리·`/api/spreads` api 분기·`stop redis` 503 로컬 검증 완료(2026-09-14, :8090). EC2 공존·PR check·자동 배포·행이 있는 상태의 Redis 격리·Influx 첫 점은 GitHub 권한·EC2 대기 |
