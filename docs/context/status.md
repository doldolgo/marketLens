...
| history | `/history/*` 3종·백필 (쓰기는 없음 — persist 루프는 001 이 지웠고 009 flusher 가 새로 쓴다) | 기록 탭 (mock) | `/history/*` 실데이터 연결은 후속 |
...
| raw-archive | 없음(001 이 snapshot 루프를 지웠다 — 010 이 원문 싱크를 새로 만든다) | - | `core/s3.py` 도 010 이 다시 만든다 |
| health | /health/collect·실패 구간 추적·collect_fail 쓰기/복원 | 실데이터 탭·5초 폴링·KPI 수집 상태 | 백오프는 013 |
| binance-stream | 없음(001 이 REST 커넥터·깊이 캐시를 지웠다 — 012 가 스트림 커넥터와 심볼 집합 `ForeignSymbolSource` 를 새로 만든다) | - | 그 전까지 바이낸스 행 없음·우주 비어 있음 |

## 알려진 빚
- (001) `server/build/`(setuptools 산출물 76파일)가 git 에 추적돼 있다 — ruff 기본 제외라 검증엔 무해하지만 별도 chore 로 지울 것.
