...
## S3 원문 아카이브
- 객체 키 `raw/exchange=<id>/dt=YYYY-MM-DD/hh=HH/YYYYMMDDTHHMMSS.mmmZ.jsonl.gz`(UTC, 시각 = 객체 첫 줄의 수신 시각). 거래소마다 60초 또는 비압축 32MB 마다 객체 1개.
...
- lifecycle 미설정(무제한). 원문 유입량 실측(001 §5, 국내 둘만, 비압축): 업비트 ≈175 msg/s·440 KB/s, 빗썸 ≈765 msg/s·1,020 KB/s → 합계 ≈1.4 MB/s ≈ **130 GB/일**(바이낸스 제외). gzip 후 하루 10~20GB 는 이 비압축량에 대한 추정이다. 보존 기간을 정하면 버킷 설정으로 — 코드는 관여하지 않는다.
...
- 원문 닫기 회차(010, 매초) + 업로드 워커(스레드 1개): 거래소별 버퍼가 60초(첫 줄 수신 시각 기준) 또는 비압축 32MB 에 닿으면 닫아 gzip 해 대기열에 넣고, 워커가 머리부터 `PutObject`. 닫기는 업로드와 독립이라 S3 가 막혀도 열린 버퍼는 32MB + 한 회차치 유입(1초 + gzip 레벨 6 으로 0.3초 안팎) 이상 자라지 않는다. 실패한 객체는 대기열 머리에 남아 1초 뒤 같은 키로 재시도, 대기열은 압축 후 256MB 를 넘으면 오래된 객체부터 버리고 에러 로그. 종료 시 열린 버퍼를 닫아 넣고 워커가 비우기를 5초 안에서 기다린다 — 못 올린 것은 버린다.
