...
- 원문 업로드 루프(010, 매초): 거래소별 버퍼가 60초(첫 줄 수신 시각 기준) 또는 비압축 32MB 에 닿으면 닫아 gzip → `PutObject`. 실패한 객체는 대기열 머리에 남아 다음 회차에 같은 키로 재시도, 대기열은 압축 후 256MB 를 넘으면 오래된 객체부터 버리고 에러 로그. 종료 시 열린 버퍼를 닫아 5초 안에서 올린다.
