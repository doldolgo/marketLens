...
- 원문 닫기 회차(010, 매초) + 업로드 워커(스레드 1개): 거래소별 버퍼가 60초(첫 줄 수신 시각 기준) 또는 비압축 32MB 에 닿으면 닫아 gzip 해 대기열에 넣고, 워커가 머리부터 `PutObject`. 닫기는 업로드와 독립이라 S3 가 막혀도 열린 버퍼는 32MB 를 넘지 않는다. 실패한 객체는 대기열 머리에 남아 1초 뒤 같은 키로 재시도, 대기열은 압축 후 256MB 를 넘으면 오래된 객체부터 버리고 에러 로그. 종료 시 열린 버퍼를 닫아 넣고 워커가 비우기를 5초 안에서 기다린다 — 못 올린 것은 버린다.
