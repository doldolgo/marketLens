...
- Redis 스트림 `ticks` 는 flusher 만 읽는다. 채널 `spreads`·키 `spreads:latest` 는 api 의 구독 허브와 `GET /spreads`(두 역할, 018)가, 키 `spreads:want` 는 수집의 게시기가 읽는다(017). S3 를 읽는 코드는 없다.
